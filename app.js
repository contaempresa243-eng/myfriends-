  ...
  unsubscribeGruposComunidade = db.collection('chats')
    ...
    .onSnapshot((snapshot) => {
      ...
      function criarItemChat(doc, ehAnuncio) {
        ...
        const avatarConteudo = chat.foto
          ? '<img src="' + chat.foto + '" style="width:100%; height:100%; object-fit:cover; border-radius:50%;">'
          : (ehAnuncio ? '<i class="fa-solid fa-bullhorn" style="font-size:16px;"></i>' : '<i class="fa-solid fa-comment" style="font-size:16px;"></i>');
        item.innerHTML =
          '<div class="avatar" style="background:' + (ehAnuncio ? '#00a884' : '#667781') + '; overflow:hidden;">' +
          avatarConteudo + '</div>' +
...
let criarGrupoFotoFicheiro = null;
...
function atualizarContadorGrupo() {
  const nome = document.getElementById('criar-grupo-nome').value;
  document.getElementById('criar-grupo-nome-contador').innerText = nome.length + '/100';
}

document.addEventListener('DOMContentLoaded', () => {
  const inputFotoGrupo = document.getElementById('criar-grupo-foto-input');
  if (inputFotoGrupo) {
    inputFotoGrupo.addEventListener('change', (e) => {
      const file = e.target.files[0];
      if (!file) return;
      criarGrupoFotoFicheiro = file;
      const preview = document.getElementById('criar-grupo-foto-preview');
      preview.innerHTML = '<img src="' + URL.createObjectURL(file) + '" style="width:100%; height:100%; object-fit:cover;">';
    });
  }
});
...
function criarGrupoComunidade() {
  ...
  if (!nome) {
    status.style.color = '#f15c6d';
    status.innerText = 'Escreve um nome para o grupo.';
    return;
  }
  ...
  const enviarFoto = criarGrupoFotoFicheiro
    ? (() => {
        const formData = new FormData();
        formData.append('file', criarGrupoFotoFicheiro);
        formData.append('upload_preset', CLOUDINARY_UPLOAD_PRESET);
        return fetch('https://api.cloudinary.com/v1_1/' + CLOUDINARY_CLOUD_NAME + '/auto/upload', { method: 'POST', body: formData })
          .then((res) => res.json())
          .then((data) => data.secure_url || null)
  ...
  enviarFoto.then((fotoUrl) => db.collection('chats').add({
    ...
    .then(() => fecharCriarGrupo())
    .catch((err) => {
      console.error('Erro ao criar grupo:', err);
      status.style.color = '#f15c6d';
      status.innerText = 'Não foi possível criar (' + (err.code || err.message || 'erro') + ').';
    });
}
...
function mostrarEcraChamadaRecebida(chamada) {
  ...
  iniciarToqueChamadaRecebida();
}

// Toca o som/vibração escolhidos para chamadas, repetindo enquanto o telefone "toca"
let intervaloToqueChamada = null;

function iniciarToqueChamadaRecebida() {
  pararToqueChamadaRecebida();

  const tocar = () => {
    const toqueId = window.localStorage.getItem('myfriens_notif_toque_chamada_' + currentChatId) || 'predefinido';
    const opcaoToque = opcoesToque.find((o) => o.id === toqueId);
    if (opcaoToque) tocarTomPreview(opcaoToque.freq, opcaoToque.duracao);

    const vibracaoId = window.localStorage.getItem('myfriens_notif_vibracao_chamada_' + currentChatId) || 'predefinicao';
    const opcaoVibracao = opcoesVibracao.find((o) => o.id === vibracaoId);
    if (opcaoVibracao && navigator.vibrate) navigator.vibrate(opcaoVibracao.padrao);
  };

  tocar();
  intervaloToqueChamada = setInterval(tocar, 2000);
}

function pararToqueChamadaRecebida() {
  if (intervaloToqueChamada) {
    clearInterval(intervaloToqueChamada);
    intervaloToqueChamada = null;
  }
...
function abrirDetalhesGrupo() {
  ...
  const chatIdDetalhes = currentChatId;
  ...
  db.collection('chats').doc(chatIdDetalhes).get().then((doc) => {
    const chat = doc.data() || {};
    const membros = chat.participantes || [];
    ...
    avatarDiv.innerHTML = chat.foto
      ? '<img src="' + chat.foto + '" style="width:100%; height:100%; object-fit:cover;">'
      : '<span class="fa-solid fa-users" style="color:#667781; font-size:40px;"></span>';

    const listaMembros = document.getElementById('detalhes-grupo-membros-lista');
    ...
    membros.forEach((email) => {
      ...
      item.innerHTML =
        '<div class="avatar" style="background:#4a90d9;">' + nome.substring(0, 2).toUpperCase() + '</div>' +
        '<div class="chat-info"><h4>' + (email === myEmail ? 'Eu' : nome) + (ehAdmin ? ' <span style="color:var(--whatsapp-teal); font-size:11px;">· admin</span>' : '') + '</h4><p>' + email + '</p></div>';
    ...
    if (chat.comunidadeId) {
      ...
      db.collection('comunidades').doc(chat.comunidadeId).get().then((comDoc) => {
        ...
        db.collection('chats').where('comunidadeId', '==', chat.comunidadeId).get().then((snap) => {
          document.getElementById('detalhes-grupo-comunidade-grupos').innerText = 'Comunidade · ' + snap.size + ' grupos';
        });
      });
    } else {
...
function fecharDetalhesGrupo() {
  document.getElementById('group-details-screen').classList.add('hidden');
}

document.addEventListener('DOMContentLoaded', () => {
  const inputFotoGrupo = document.getElementById('detalhes-grupo-foto-input');
  if (inputFotoGrupo) {
    inputFotoGrupo.addEventListener('change', (e) => {
      const file = e.target.files[0];
      e.target.value = '';
      ...
      const avatarDiv = document.getElementById('detalhes-grupo-avatar');
      avatarDiv.innerHTML = '<span class="fa-solid fa-spinner fa-spin" style="color:#667781; font-size:24px;"></span>';
      ...
      fetch('https://api.cloudinary.com/v1_1/' + CLOUDINARY_CLOUD_NAME + '/auto/upload', { method: 'POST', body: formData })
        ...
        .then((data) => {
          ...
          return db.collection('chats').doc(currentChatId).set({ foto: data.secure_url }, { merge: true })
            .then(() => data.secure_url);
        })
        .then((url) => {
          avatarDiv.innerHTML = '<img src="' + url + '" style="width:100%; height:100%; object-fit:cover;">';
        })
        .catch((err) => {
          console.error('Erro ao atualizar foto do grupo:', err);
          avatarDiv.innerHTML = '<span class="fa-solid fa-users" style="color:#667781; font-size:40px;"></span>';
...
function abrirPermissoesGrupo() {
  fecharDetalhesGrupo();
  document.getElementById('permissoes-grupo-subtitulo').innerText = chatNameAtual;
  document.getElementById('group-permissions-screen').classList.remove('hidden');

  db.collection('chats').doc(currentChatId).get().then((doc) => {
    const p = (doc.exists && doc.data().permissoes) || {};
    document.getElementById('perm-editar').checked = p.editarDefinicoes !== false;
    document.getElementById('perm-enviar').checked = p.enviarMensagens !== false;
    document.getElementById('perm-adicionar').checked = p.adicionarMembros !== false;
    document.getElementById('perm-historico').checked = p.enviarHistorico !== false;
    document.getElementById('perm-aprovar').checked = !!p.aprovarNovosMembros;
  }).catch((err) => console.error('Erro ao carregar permissões:', err));
}

function fecharPermissoesGrupo() {
  document.getElementById('group-permissions-screen').classList.add('hidden');
}

function guardarPermissoesGrupo() {
  const permissoes = {
    editarDefinicoes: document.getElementById('perm-editar').checked,
    enviarMensagens: document.getElementById('perm-enviar').checked,
    adicionarMembros: document.getElementById('perm-adicionar').checked,
    enviarHistorico: document.getElementById('perm-historico').checked,
    aprovarNovosMembros: document.getElementById('perm-aprovar').checked
  };
  db.collection('chats').doc(currentChatId).set({ permissoes: permissoes }, { merge: true })
...
function abrirNotificacoesGrupo() {
  ...
  const vibracaoId = window.localStorage.getItem('myfriens_notif_vibracao_' + currentChatId) || 'predefinicao';
  document.getElementById('notif-vibracao-label').innerText = opcoesVibracao.find((o) => o.id === vibracaoId).nome;

  const toqueId = window.localStorage.getItem('myfriens_notif_toque_' + currentChatId) || 'predefinido';
  document.getElementById('notif-toque-label').innerText = opcoesToque.find((o) => o.id === toqueId).nome;

  const vibracaoChamadaId = window.localStorage.getItem('myfriens_notif_vibracao_chamada_' + currentChatId) || 'predefinicao';
  document.getElementById('notif-vibracao-chamada-label').innerText = opcoesVibracao.find((o) => o.id === vibracaoChamadaId).nome;

  const toqueChamadaId = window.localStorage.getItem('myfriens_notif_toque_chamada_' + currentChatId) || 'predefinido';
  document.getElementById('notif-toque-chamada-label').innerText = opcoesToque.find((o) => o.id === toqueChamadaId).nome;

  document.getElementById('group-notifications-screen').classList.remove('hidden');
}

function fecharNotificacoesGrupo() {
...
function abrirNotificarModal() {
  const atual = window.localStorage.getItem('myfriens_notif_tipo_' + currentChatId) || 'todas';
  atualizarRadiosNotificar(atual);
  document.getElementById('notificar-modal').classList.remove('hidden');
}

function atualizarRadiosNotificar(tipo) {
  document.getElementById('radio-notificar-todas').className = tipo === 'todas' ? 'fa-solid fa-circle-dot' : 'fa-regular fa-circle';
  document.getElementById('radio-notificar-todas').style.color = tipo === 'todas' ? 'var(--whatsapp-teal)' : '#667781';
  document.getElementById('radio-notificar-destaques').className = tipo === 'destaques' ? 'fa-solid fa-circle-dot' : 'fa-regular fa-circle';
  document.getElementById('radio-notificar-destaques').style.color = tipo === 'destaques' ? 'var(--whatsapp-teal)' : '#667781';
}

function selecionarNotificar(tipo) {
  window.localStorage.setItem('myfriens_notif_tipo_' + currentChatId, tipo);
  document.getElementById('notif-tipo-label').innerText = tipo === 'todas' ? 'Todas' : 'Destaques';
  document.getElementById('notificar-modal').classList.add('hidden');
}

// ---- Vibração (com pré-visualização real via navigator.vibrate) ----
const opcoesVibracao = [
  { id: 'desligada', nome: 'Desligada', padrao: [] },
  { id: 'predefinicao', nome: 'Predefinição', padrao: [200] },
  { id: 'curta', nome: 'Curta', padrao: [80] },
  { id: 'longa', nome: 'Longa', padrao: [500] }
];

let contextoVibracaoAtual = 'mensagem';

function abrirVibracaoModal(contexto) {
  contextoVibracaoAtual = contexto || 'mensagem';
  const chave = contextoVibracaoAtual === 'chamada' ? 'myfriens_notif_vibracao_chamada_' : 'myfriens_notif_vibracao_';
  const atual = window.localStorage.getItem(chave + currentChatId) || 'predefinicao';
  const lista = document.getElementById('lista-vibracao-opcoes');
  lista.innerHTML = '';

  opcoesVibracao.forEach((op) => {
    const item = document.createElement('div');
    item.style.cssText = 'display:flex; align-items:center; gap:14px; padding:12px 20px; cursor:pointer;';
    const marcado = op.id === atual;
    item.innerHTML =
      '<span class="fa-solid ' + (marcado ? 'fa-circle-dot' : 'fa-circle') + '" style="color:' + (marcado ? 'var(--whatsapp-teal)' : '#aaa') + '; font-size:17px;"></span>' +
      '<span style="color:#111; font-size:15px;">' + op.nome + '</span>';
    item.onclick = () => selecionarVibracao(op.id);
    lista.appendChild(item);
  });

  document.getElementById('vibracao-modal').classList.remove('hidden');
}

function fecharVibracaoModal() {
  document.getElementById('vibracao-modal').classList.add('hidden');
}

function selecionarVibracao(id) {
  const chave = contextoVibracaoAtual === 'chamada' ? 'myfriens_notif_vibracao_chamada_' : 'myfriens_notif_vibracao_';
  const labelId = contextoVibracaoAtual === 'chamada' ? 'notif-vibracao-chamada-label' : 'notif-vibracao-label';
  window.localStorage.setItem(chave + currentChatId, id);
  document.getElementById(labelId).innerText = opcoesVibracao.find((o) => o.id === id).nome;

  if (navigator.vibrate) {
    const op = opcoesVibracao.find((o) => o.id === id);
    navigator.vibrate(op.padrao);
  }

  fecharVibracaoModal();
}

// ---- Toque das notificações (sons curtos gerados via Web Audio) ----
const opcoesToque = [
  { id: 'predefinido', nome: 'Predefinido', freq: 880, duracao: 0.15 },
  { id: 'nenhum', nome: 'Nenhum', freq: null },
  { id: 'sino', nome: 'Sino', freq: 1200, duracao: 0.3 },
  { id: 'suave', nome: 'Suave', freq: 660, duracao: 0.2 },
  { id: 'assobio', nome: 'Assobio', freq: 1500, duracao: 0.35 },
  { id: 'classico', nome: 'Notificação Clássica', freq: 520, duracao: 0.12 }
];

function tocarTomPreview(freq, duracao) {
  if (!freq) return;
  try {
...
let contextoToqueAtual = 'mensagem';

function abrirToqueNotificacao(contexto) {
  contextoToqueAtual = contexto || 'mensagem';
  const chave = contextoToqueAtual === 'chamada' ? 'myfriens_notif_toque_chamada_' : 'myfriens_notif_toque_';
  const atual = window.localStorage.getItem(chave + currentChatId) || 'predefinido';
  const lista = document.getElementById('lista-toque-opcoes');
  lista.innerHTML = '';

  opcoesToque.forEach((op) => {
    const item = document.createElement('div');
    item.style.cssText = 'display:flex; align-items:center; gap:16px; padding:14px 20px; cursor:pointer; border-bottom:1px solid #eee;';
    const marcado = op.id === atual;
    item.innerHTML =
      '<span class="fa-solid ' + (marcado ? 'fa-circle-dot' : 'fa-circle') + '" style="color:' + (marcado ? '#0a84ff' : '#ccc') + '; font-size:17px;"></span>' +
      '<span style="color:#111; font-size:15px;">' + op.nome + '</span>';
    item.onclick = () => {
      tocarTomPreview(op.freq, op.duracao);
      selecionarToque(op.id, op.nome);
    };
    lista.appendChild(item);
  });

  document.getElementById('toque-notificacao-screen').classList.remove('hidden');
}

function fecharToqueNotificacao() {
  document.getElementById('toque-notificacao-screen').classList.add('hidden');
}

function selecionarToque(id, nome) {
  const chave = contextoToqueAtual === 'chamada' ? 'myfriens_notif_toque_chamada_' : 'myfriens_notif_toque_';
  const labelId = contextoToqueAtual === 'chamada' ? 'notif-toque-chamada-label' : 'notif-toque-label';
  window.localStorage.setItem(chave + currentChatId, id);
  document.getElementById(labelId).innerText = nome;
  abrirToqueNotificacao(contextoToqueAtual); // redesenha para atualizar o marcador selecionado
}

// ---- Motor de notificações: dispara som/vibração/aviso quando chega mensagem nova ----
// Funciona só enquanto esta aba do myFriens estiver aberta (sem push do sistema operativo).
let unsubscribeChatsParaNotificar = null;
const listenersMensagensPorChat = {};
const primeiraCargaPorChat = {};

function iniciarSistemaNotificacoes() {
  const myEmail = getCurrentUserEmail();
  if (!myEmail) return;

  if (unsubscribeChatsParaNotificar) unsubscribeChatsParaNotificar();

  unsubscribeChatsParaNotificar = db.collection('chats')
    .where('participantes', 'array-contains', myEmail)
    .onSnapshot((snapshot) => {
      ...
      snapshot.docs.forEach((doc) => {
        const chatId = doc.id;
        idsAtuais.add(chatId);
        if (listenersMensagensPorChat[chatId]) return; // já a escutar este chat

        primeiraCargaPorChat[chatId] = true;
        listenersMensagensPorChat[chatId] = db.collection('chats').doc(chatId).collection('messages')
          .orderBy('timestamp', 'desc')
          .limit(1)
          .onSnapshot((msgSnapshot) => {
            if (primeiraCargaPorChat[chatId]) {
              primeiraCargaPorChat[chatId] = false; // ignora a mensagem já existente na primeira leitura
              return;
            }
            msgSnapshot.docChanges().forEach((change) => {
              if (change.type === 'added') {
                processarNotificacaoRecebida(chatId, change.doc.data());
              }
            });
          }, (err) => console.error('Erro ao escutar mensagens para notificação:', err));
      });

      // Deixa de escutar chats de que já não fazemos parte
      Object.keys(listenersMensagensPorChat).forEach((chatId) => {
        if (!idsAtuais.has(chatId)) {
          listenersMensagensPorChat[chatId]();
          delete listenersMensagensPorChat[chatId];
          delete primeiraCargaPorChat[chatId];
        }
      });
    }, (err) => console.error('Erro ao escutar lista de chats para notificação:', err));
}

function pararSistemaNotificacoes() {
  if (unsubscribeChatsParaNotificar) { unsubscribeChatsParaNotificar(); unsubscribeChatsParaNotificar = null; }
  Object.keys(listenersMensagensPorChat).forEach((chatId) => listenersMensagensPorChat[chatId]());
  Object.keys(listenersMensagensPorChat).forEach((chatId) => delete listenersMensagensPorChat[chatId]);
  Object.keys(primeiraCargaPorChat).forEach((chatId) => delete primeiraCargaPorChat[chatId]);
}

function processarNotificacaoRecebida(chatId, msg) {
  const myEmail = getCurrentUserEmail();
  ...
  if (tipo === 'destaques') {
    const meuNome = (myEmail || '').split('@')[0].toLowerCase();
    const mencionado = msg.text && (msg.text.toLowerCase().includes('@todos') || msg.text.toLowerCase().includes('@' + meuNome));
    const respostaDireta = msg.replyTo && msg.replyTo.sender === myEmail;
    if (!mencionado && !respostaDireta) return;
  }

  const toqueId = window.localStorage.getItem('myfriens_notif_toque_' + chatId) || 'predefinido';
  const opcaoToque = opcoesToque.find((o) => o.id === toqueId);
  if (opcaoToque) tocarTomPreview(opcaoToque.freq, opcaoToque.duracao);

  const vibracaoId = window.localStorage.getItem('myfriens_notif_vibracao_' + chatId) || 'predefinicao';
  const opcaoVibracao = opcoesVibracao.find((o) => o.id === vibracaoId);
  if (opcaoVibracao && navigator.vibrate) navigator.vibrate(opcaoVibracao.padrao);

  const remetente = (msg.sender || '').split('@')[0];
  mostrarToast(remetente + ': ' + previaMensagem(msg));
}

function abrirPesquisaDeDetalhes() {
  fecharDetalhesGrupo();
  abrirPesquisaMensagens();
}

function abrirAdicionarDeDetalhes() {
  fecharDetalhesGrupo();
  abrirAdicionarMembro();
}

function voltarParaComunidadeDeDetalhes() {
  fecharDetalhesGrupo();
  if (!comunidadeIdAtual) return;
  db.collection('comunidades').doc(comunidadeIdAtual).get().then((doc) => {
    if (doc.exists) {
      closeChat();
      abrirComunidade(doc.id, doc.data());
    }
  });
}
